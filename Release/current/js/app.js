...
app.controller('MainCtrl', [
  ...
  '$http',
  '$sce',
  function($scope, $timeout,$http,$sce){
    ...
    $scope.toCloseACPopovers = [];
    $scope.toCloseNCPopovers = [];
    $scope.toCloseARPopovers = [];
    $scope.displayResponsePopover = function(speech, item, evt, indexAC, indexNC, indexAR){
      var pop = $(evt.target).closest('.argText');
      pop.initPopover(indexAC, indexNC, indexAR, item.response, speech);
      pop.popover('show');
      if(speech == 1)
        $scope.toCloseACPopovers[indexAC] = pop;
      else if(speech == 2)
        $scope.toCloseNCPopovers[indexNC] = pop;
      else if(speech == 3)
        $scope.toCloseARPopovers[indexAR] = pop;
